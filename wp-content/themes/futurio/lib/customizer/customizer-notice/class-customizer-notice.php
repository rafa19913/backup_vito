<?php
/**
 * Futurio Customizer Notification System.
 *
 * @package Futurio
 */
...
/**
 * Futurio Customizer Notify Class
 */
class Futurio_Customizer_Notice {
	...
	/**
	 * The single instance of Futurio_Customizer_Notice
	 *
	 * @var Futurio_Customizer_Notice $instance The Futurio_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main Futurio_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of Futurio_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Futurio_Customizer_Notice ) ) {
			self::$instance = new Futurio_Customizer_Notice;
	...
	public function setup_config() {
		...
		global $futurio_recommended_plugins;
		...
		$futurio_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$futurio_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'futurio-customizer-notice', get_template_directory_uri() . '/lib/customizer/customizer-notice/css/customizer-notice.css', array() );
		...
		wp_enqueue_script( 'futurio-customizer-notice', get_template_directory_uri() . '/lib/customizer/customizer-notice/js/customizer-notice.js', array( 'customize-controls' ) );
		wp_localize_script(
			'futurio-customizer-notice', 'customizer_notice_data', array(
				'ajaxurl'                  => admin_url( 'admin-ajax.php' ),
				'template_directory'       => get_template_directory_uri(),
				'base_path'                => admin_url(),
				'activating_string'        => esc_html__( 'Activating', 'futurio' ),
			)
		);
	...
	public function customize_register_notice( $wp_customize ) {
		...
		/**
		 * Include the Futurio_Customizer_Notice_Section class.
		 */
		require_once get_template_directory() . '/lib/customizer/customizer-notice/class-customizer-notice-section.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound

		$wp_customize->register_section_type( 'Futurio_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new Futurio_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			$show_recommended_plugins = get_option( 'futurio_show_recommended_plugins' );
			if ( isset( $_GET['todo'] ) ) {
				switch ( $_GET['todo'] ) {
					case 'add':
			...
			update_option( 'futurio_show_recommended_plugins', $show_recommended_plugins );
		}
