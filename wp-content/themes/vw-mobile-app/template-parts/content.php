...
<article id="post-<?php the_ID(); ?>" <?php post_class('inner-service'); ?>>
  <div class="post-main-box wow bounceInDown delay-1000" data-wow-duration="2s">
    ...
    if($vw_mobile_app_theme_lay == 'Default'){ ?>
      <div class="row">
        <?php if(has_post_thumbnail() && get_theme_mod( 'vw_mobile_app_featured_image_hide_show',true) != '') {?>
          <div class="box-image col-lg-6 col-md-6">
            <?php the_post_thumbnail(); ?>
          </div>
        <?php } ?>
        <div class="new-text <?php if(has_post_thumbnail() && get_theme_mod( 'vw_mobile_app_featured_image_hide_show',true) != '') { ?>col-lg-6 col-md-6"<?php } else { ?>col-lg-12 col-md-12 <?php } ?>">
          <h2 class="section-title"><a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php echo the_title_attribute(); ?>"><?php the_title();?><span class="screen-reader-text"><?php the_title(); ?></span></a></h2>
          <?php if( get_theme_mod( 'vw_mobile_app_toggle_postdate',true) != '' || get_theme_mod( 'vw_mobile_app_toggle_author',true) != '' || get_theme_mod( 'vw_mobile_app_toggle_comments',true) != '' || get_theme_mod( 'vw_mobile_app_toggle_time',true) != '') { ?>
            <div class="post-info">
              ...
              <?php if(get_theme_mod('vw_mobile_app_toggle_author',true)==1){ ?>
                <span><?php echo esc_html(get_theme_mod('vw_mobile_app_meta_field_separator', '|'));?></span> <i class="far fa-user"></i><span class="entry-author"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php the_author(); ?><span class="screen-reader-text"><?php the_author(); ?></span></a></span>
              <?php } ?>

              <?php if(get_theme_mod('vw_mobile_app_toggle_comments',true)==1){ ?>
                <span><?php echo esc_html(get_theme_mod('vw_mobile_app_meta_field_separator', '|'));?></span> <i class="fa fa-comments" aria-hidden="true"></i><span class="entry-comments"><?php comments_number( __('0 Comment', 'vw-mobile-app'), __('0 Comments', 'vw-mobile-app'), __('% Comments', 'vw-mobile-app') ); ?> </span>
              <?php } ?>

              <?php if(get_theme_mod('vw_mobile_app_toggle_time',true)==1){ ?>
                <span><?php echo esc_html(get_theme_mod('vw_mobile_app_meta_field_separator', '|'));?></span> <i class="<?php echo esc_attr(get_theme_mod('vw_mobile_app_toggle_time_icon','far fa-clock')); ?>"></i><span class="entry-time"><?php echo esc_html( get_the_time() ); ?></span>
              <?php } ?>
              <hr>
            </div>  
          <?php } ?>    
    ...
    <?php }else if($vw_mobile_app_theme_lay == 'Center'){ ?>
      <div class="service-text">
        <?php if( get_theme_mod( 'vw_mobile_app_featured_image_hide_show',true) != '') { ?>
          <div class="box-image">
            <?php the_post_thumbnail(); ?>
          </div>
        <?php } ?>
